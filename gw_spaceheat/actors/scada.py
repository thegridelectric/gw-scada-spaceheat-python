...
from named_types import (AdminDispatch, AdminKeepAlive, AdminReleaseControl, AllyGivesUp, ChannelFlatlined, 
                        ...
                        GameOn, GoDormant, 
                        LayoutLite, NewCommandTree, RemainingElec,  RemainingElecEvent,
                        ScadaParams, SendLayout, SingleMachineState, SuitUp, WakeUp)
...
class Scada(ScadaInterface, Proactor):
    ...
    def admin_keep_alive_received(self, from_node: ShNode, payload: AdminKeepAlive) -> None:
        if from_node != self.admin:
    ...
    def params_authorized(self, msg: ScadaParams) -> bool:
        """Verify that the params were sent by this Scada's AtomicTNode
        """
        # TODO: add signature
        if msg.FromGNodeAlias != self.hardware_layout.atn_g_node_alias:
            self.log(f"Not the correct GNodeAlias for ScadaParams: {msg.FromGNodeAlias}")
            return False
        return True
    ...
    def scada_params_received(self, from_node: ShNode, message: ScadaParams) -> None:
        ...
        ScadaParams (with old and new)

        """
        if not self.params_authorized(message):
            return
    ...
    def remaining_elec_received(self, from_node: ShNode, msg: RemainingElec) -> None:
        ...
        if from_node.Name != H0N.synth_generator:
            self.log(f"Ignoring RemainingElecReceived from {from_node.Name} - expect {H0N.synth_generator}")
        self._send_to(self.atomic_ally, msg)
        ...
        if to_node.name in set(self._communicators.keys()):
            ...
            self._links.publish_message(
                ...
                message = Message(
                    ...
                    Dst=self._links._mqtt_clients.upstream_topic_dst,
                    Payload=payload,
                ),
                qos=QOS.AtMostOnce
            )
    ...
    def ally_gives_up_received(self, from_node: ShNode, msg: AllyGivesUp) -> None:
        if from_node.Name != H0N.atomic_ally:
            self.log(f"Ignoring AllyGivesUp from {from_node.Name} - expect AtomicAlly (aa)")
            return
        if self.auto_state != MainAutoState.Atn:
    ...
    def suit_up_received(self, from_node: ShNode, msg: SuitUp) -> None:
        if from_node.Name != H0N.atomic_ally:
            self.log(f"Ignoring AllyGivesUp from {from_node.Name} - expect AtomicAlly (aa)")
            return
