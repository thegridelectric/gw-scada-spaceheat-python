    ...
    def derived_errors(self) -> List[str]:
        ...
        if self.DefaultBaud:
            if not isinstance(self.DefaultBaud, int):
                errors.append(f"DefaultBaud {self.DefaultBaud} must have type int.")
        if self.UpdatePeriodMs:
            if not isinstance(self.UpdatePeriodMs, int):
                errors.append(f"UpdatePeriodMs {self.UpdatePeriodMs} must have type int.")
        if not isinstance(self.LocalCommInterface, LocalCommInterface):
            errors.append(f"LocalCommInterface {self.LocalCommInterface} must have type {LocalCommInterface}.")
