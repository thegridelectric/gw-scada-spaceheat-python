...
from schema.gt.gt_powermeter_reporting_config.gt_powermeter_reporting_config_base import GtPowermeterReportingConfigBase
...
class GtPowermeterReportingConfig(GtPowermeterReportingConfigBase):
    ...
    def check_for_errors(self):
        ...
        if len(errors) > 0:
            raise MpSchemaError(f" Errors making making gt.powermeter.reporting.config for {self}: {errors}")
    ...
    def poll_more_than_reporting_errors(self) -> list:
        ...
        if self.PollPeriodMs < 1000 * self.ReportingPeriodS:
            ...
            return [f"Error! self.PollPeriodMs {self.PollPeriodMs} is greater than "
