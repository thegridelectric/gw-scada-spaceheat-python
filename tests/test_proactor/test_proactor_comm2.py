...
@pytest.mark.skip()
@pytest.mark.asyncio
async def test_broker_dns_failure():
...
async def test_no_parent():
    async with CommTestHelper2(add_child=True) as h:
        child = h.child
...
@pytest.mark.asyncio
async def test_basic_comm_child_first():
    async with CommTestHelper2(add_child=True, add_parent=True, verbose=True) as h:
        child = h.child
...
@pytest.mark.asyncio
async def test_basic_parent_comm_loss():
    async with CommTestHelper2(add_child=True, add_parent=True, verbose=True) as h:
        child = h.child
        child_stats = child.stats.link(child.upstream_client)
        child_comm_event_counts = child_stats.comm_event_counts
...
@pytest.mark.asyncio
async def test_awaiting_setup_and_peer2():
    """
    Test:
    ...
    """
    async with CommTestHelper2(add_child=True, verbose=True) as h:
        child = h.child
        stats = child.stats.link(child.upstream_client)
        comm_event_counts = stats.comm_event_counts
        link = child._link_states.link(child.upstream_client)
...
@pytest.mark.asyncio
async def test_awaiting_setup_and_peer_corner_cases2():
    """
    Test corner cases:
     (connecting -> connected -> awaiting_setup_and_peer)
...
# @pytest.mark.skip
@pytest.mark.asyncio
async def test_awaiting_setup2__(request):
    """
...
@pytest.mark.asyncio
async def test_response_timeout2():
    """
    Test:
...
async def test_ping2():
    ...
    Test:
        ...
        ping restores comm
    """
    child_settings = CommTestHelper2.default_child_settings()
    parent_settings = CommTestHelper2.default_parent_settings()
